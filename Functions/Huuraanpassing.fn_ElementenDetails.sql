...
CREATE function [Huuraanpassing].[fn_ElementenDetails] (@Eenheidnr NVARCHAR(20), @Peildatum date) 
returns table 
as
/* ###################################################################################################
VAN         : JvdW
...
SELECT Eenheidnr = CONTR.Eenheidnr_
	...
	,Elementnr = E.[Nr_]
	,Bedrag = E.[Bedrag (LV)]
	,[Bedrag incl. BTW] = round( E.[bedrag (lv)] * (100.0 + isnull(VPS.[Vat _], 0)) / (100.0 * 1), 2) 
    ,[Bedrag niet afgerond] = E.[eenheidprijs (LV)]  -- select * from empire.empire.dbo.[Staedion$Element] where Eenheidnr_ = 'OGEH-0055183'
	,E.[BTW-productboekingsgroep]
	,Eenmalig = E.Eenmalig
	,[Omschrijving element] = E.omschrijving
	,E.[Op huurverhoging afdrukken]
FROM staedion_dm.[Huuraanpassing].[staedion$contract] AS CONTR
...
INNER JOIN staedion_dm.[Huuraanpassing].[Staedion$Element] AS E ON CONTR.[Eenheidnr_] = E.[Eenheidnr_]
	AND CONTR.[Volgnr_] = E.[Volgnummer]
JOIN CTE_peildata AS P ON 1 = 1
LEFT OUTER JOIN staedion_dm.[Huuraanpassing].[Staedion$VAT Posting Setup] as VPS ON VPS.[VAT Bus_ Posting Group] = 'NL'
