...
CREATE function [Elementen].[Details] (@Eenheidnr NVARCHAR(20), @Elementnr NVARCHAR(3), @Peildatum date) 
returns table 
as
/* ###################################################################################################
VAN         : JvdW
...
from staedion_dm.[Elementen].[Details]('20210228')
WHERE Elementnr = '504'
...
WHERE CONTR.[Ingangsdatum] <= (
	...
	AND (E.[Nr_] = @Elementnr OR @Elementnr IS NULL)
GO
