...
SET ANSI_NULLS ON
GO
create function [Eenheden].[fnOgeKenmerken TEST] (@Eenheidnr varchar(20), @Peildatum date = null)
returns table
as
/* ########################################################################################################################## 
VAN 		  JvdW
...
--------------------------------------------------------------------------------------------------------------------------
-- per eenheid
select * from [Eenheden].[fnOgeKenmerken] ( 'ADEH-0050003', getdate())
...
SELECT Eenheidnr = Nr_
       ...
       ,BEH.Beheerder 
			 ...
			 ,[Eenheidtype Corpodata] = TT.[Analysis Group Code]
FROM empire_data.dbo.staedion$oge AS OGE
LEFT OUTER JOIN empire_data.dbo.Staedion$Type AS TT
ON TT.[Code] = OGE.[Type] 
AND TT.Soort <> 2 -- collectief object
