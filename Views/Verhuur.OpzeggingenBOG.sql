...
SET ANSI_NULLS ON
GO
CREATE VIEW [Verhuur].[OpzeggingenBOG]
AS
...
SELECT Verhuurteam = E.staedion_verhuurteam,
	   ...
	   [Netto huur incl btw] = E.[nettohuur_incl_btw],
...
FROM backup_empire_Dwh.dbo.d_opzegging AS D
	...
	LEFT OUTER JOIN backup_empire_Dwh.dbo.cluster AS C
		ON C.id = E.[staedion_fk_ftcluster_id]
	LEFT OUTER JOIN backup_Empire_Dwh.dbo.wijk AS W
		ON W.id = E.fk_wijk_id
