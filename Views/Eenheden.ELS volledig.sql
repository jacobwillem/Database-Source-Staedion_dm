...
CREATE VIEW [Eenheden].[ELS volledig]
AS

/* #############################################################################################################################
EXEC sys.sp_addextendedproperty @name = N'MS_Description'
       ,@value = N'Betreft de view op de ELS-lijst. Per eerste en laatste van de maand wordt de data getoond van de ELS-lijst


Gebaseerd op de oude view [Eenheden].[ELS] en de bijbehorende PBI "Complexkenmerken" in werkruimte O&V
Deze is nu vervangen door deze view en bijbehorende PBI "Complexkenmerken incl ELS-lijst" in werkruimte Staedion.
Andere objecten mbt ELS-lijst:
- empire_staedion_logic.dbo.dsp_update_ELS
-  
Rest niet meer in gebruik en wordt niet meer bijgehouden:
- empire_dwh.dbo.dsp_rs_ELS (tbv SSRS-variant)

'      ,@level0type = N'SCHEMA'
       ,@level0name = 'Eenheden'
       ,@level1type = N'VIEW'
       ,@level1name = 'ELS volledig';
GO

--------------------------------------------------------------------------------------------------------------------------------
WIJZIGINGEN
--------------------------------------------------------------------------------------------------------------------------------
20220330 Gebaseerd op eerdere view staedion_dm.Eenheden.Els van Ruben Stolk
--------------------------------------------------------------------------------------------------------------------------------
TESTEN 
--------------------------------------------------------------------------------------------------------------------------------
select datum, [datum van], [meest recent], count(*) 
from staedion_dm.[Eenheden].[ELS volledig]
group by datum, [datum van], [meest recent]
order by datum desc
;
--------------------------------------------------------------------------------------------------------------------------------
METADATA
...
GO
EXEC sp_addextendedproperty N'MS_Description', N'Betreft de view op de ELS-lijst. Per eerste en laatste van de maand wordt de data opgehaald van de ELS-lijst tbv PBI rapportage "Complexkenmerken incl ELS-lijst"
Gebaseerd op de oude view [Eenheden].[ELS] en de bijbehorende PBI "Complexkenmerken" in werkruimte O&V
Deze is nu vervangen door deze view en bijbehorende PBI "Complexkenmerken incl ELS-lijst" in werkruimte Staedion.
Andere objecten mbt ELS-lijst:
- empire_staedion_logic.dbo.dsp_update_ELS
- staedion_dm.tms.MarktwaardeHistorie
NB: in extendend properties van tabel empire_staedion_data.dbo.els taat per kolom een beschrijving.', 'SCHEMA', N'Eenheden', 'VIEW', N'ELS volledig', NULL, NULL
GO
