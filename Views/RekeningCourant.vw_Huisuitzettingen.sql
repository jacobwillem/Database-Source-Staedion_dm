...
CREATE VIEW [RekeningCourant].[vw_Huisuitzettingen] AS 
/* ##########################################################################################
Van: JvdW
...
20210824 Uitgebreid met afgelast / reden afgelast
		-- finsql: Lookup("Cause of Eviction".Description WHERE (Code=FIELD(Cause of Eviction Code)))
		-- finsql: Lookup("Reden afgelasting ontruiming".Omschrijving WHERE (Code=FIELD(Reden afgelasting)))
---------------------------------------------------------------------------------------------
TEST
---------------------------------------------------------------------------------------------
-- check aantal met jaarplan
select * from staedion_dm.RekeningCourant.Huisuitzettingen where year([Datum ontruiming]) = 2021 and month([Datum ontruiming]) < 9 and [Ontruiming afgelast] = 'Nee' and [Code ontruiming] in ('ONTRUIM11', 'ONTRUIM12', 'ONTRUIM13')
---------------------------------------------------------------------------------------------
VORIGE VERSIE
---------------------------------------------------------------------------------------------
...
SELECT AANZ.[Datum ontruiming]
	...
	,CORP.[Corpodatatype omschrijving]
FROM empire_data.dbo.Staedion$Aanzegging_ontruiming AANZ
INNER JOIN empire_data.dbo.Staedion$Deurwaarderdossier AS DD ON AANZ.Dossiernr_ = DD.Dossiernr_
LEFT OUTER JOIN staedion_dm.Eenheden.Eigenschappen AS EIG ON DD.[Eenheidnr_] = EIG.Eenheidnr
	AND EIG.Einddatum IS NULL
LEFT OUTER JOIN staedion_dm.Eenheden.Corpodatatype AS CORP ON CORP.Corpodatatype_id = EIG.Corpodatatype_id
	AND EIG.Einddatum IS NULL
--LEFT OUTER JOIN empire_dwh.dbo.[contract] AS CONTR ON DD.[Customer No_] = CONTR.fk_klant_id
--	AND DD.Eenheidnr_ = CONTR.bk_eenheidnr
LEFT OUTER JOIN empire_data.dbo.Staedion$Cause_of_Eviction AS OORZ ON OORZ.[Code] = AANZ.[Cause of Eviction Code]
