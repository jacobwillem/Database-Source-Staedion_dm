...
CREATE VIEW [Leefbaarheid].[Leefbaarheidsdossiers]
AS
WITH cte
AS (
	SELECT [No_]
		...
		,Clusternummer = CASE 
			...
			ELSE NULL
			END
		,ClusternummerOud = iif([Clusternr_] LIKE 'FINC-%', [Clusternr_], NULL)
		,Dossiertype = CASE 
...
AS (
	SELECT Clusternummer
		,Jaar
		,AantalOverlastDossiers = COUNT(No_)
	FROM cte
...
AS (
	SELECT Clusternummer
		,Jaar
		,AantalOnrechtmatigGebruikDossiers = COUNT(No_)
	FROM cte
	WHERE Clusternummer IS NOT NULL
