...
select
  ...
  Bedrag =  case
              ...
              when EOMONTH(d.datum) between [Datum start bouw PROG] and isnull(pg.[Datum oplevering PROG],'99991231') then (([Bedrag project prognose] - isnull(ct.totaal,0.00)) * lp_jaar.waarde) / (DATEDIFF(mm,empire_logic.dbo.dlf_maxdate(DATEADD(yy, DATEDIFF(yy, 0, d.datum), 0), [Datum start bouw PROG]), empire_logic.dbo.dlf_mindate(DATEADD(yy, DATEDIFF(yy, 0, d.datum) + 1, -1), isnull(pg.[Datum oplevering PROG],'99991231'))) + 1)
            end
from Liqprog.Projectgegevens as pg
join empire_dwh.dbo.tijd as d on 
  d.dag_vd_maand = 1 and
  d.datum >= DATEADD(month, DATEDIFF(month, 0, GETDATE()), 0)
