...
Q as (
	select 	 [id] = R.[id]
			...
			,[Detail] = case T.[num]
							...
							when 11 then replicate(NCHAR(8203), 366 - DATEPART(dayofyear,r.datum)) + format(R.datum, 'dd MMM', 'nl-NL')
							when 12 then replicate(NCHAR(8203), 12 - MONTH(R.datum)) + format(R.datum, 'MMM', 'nl-NL')		-- left(convert(varchar, R.[Datum], 23), 7)
							when 13 then replicate(NCHAR(8203), 3 - MONTH(R.datum)) + format(R.datum, 'MMM', 'nl-NL')		-- left(convert(varchar, R.[Datum], 23), 7)
							when 14 then replicate(NCHAR(8203), 12 - MONTH(R.datum)) + format(R.datum, 'MMM', 'nl-NL')		-- left(convert(varchar, R.[Datum], 23), 7)
							when 15 then C.[Deelgebied]
							when 16 then C.[Wijk]
							when 17 then C.[Buurt]
							when 18 then R.[Clusternummer]
							when 19 then C.[Gemeente]
						end
      ,[Datum] = R.Datum
	...
	where R.[fk_indicator_id] is not null --and fk_indicator_id between 1500 and 1550
)
