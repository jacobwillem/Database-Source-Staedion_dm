...
CREATE procedure [dbo].[sp_load_kpi_kcm_woonkwaliteit_hoger_dan_handmatig](
  @peildatum date = '20191231'
)
as
/* #################################################################################################################

exec staedion_dm.[dbo].[sp_load_kpi_kcm_woonkwaliteit_hoger_dan_handmatig] '20200131',@LoggenDetails =1
select * from empire_staedion_Data.etl.LogboekMeldingenProcedures

declare @fk_indicator_id as smallint
select @fk_indicator_id = min(id) from  [Dashboard].[Indicator] where lower([Omschrijving]) like '%eoordeelt%kwaliteit%>8%'
  ...
  -- Standaard bewaren voor kpi's de details wel
	if @LoggenDetails = 1
		begin 		
			...
			SELECT Datum, [Score woningkwaliteit >= 8], getdate(), convert(varchar(10), Datum, 120) + ' ; ' + convert(varchar(20), Tijdstip, 120) + ' ; ' + Postcode + ' ; ' + isnull(Eenheidnr, ''), @fk_indicator_id, [sleutel eenheid]
