...
CREATE PROCEDURE[dbo].[sp_load_kpi_verzuimfrequentie](
  @peildatum date = '20211231'
)
as
/* #################################################################################################################

exec staedion_dm.[dbo].[sp_load_kpi_verzuimfrequentie] '20211231'
...
BEGIN TRY
  ...
  -- Diverse variabelen
	...
	,cte8 as(
	SELECT			 [Teller] = SUM(sic_counter)
					,[Noemer] = SUM(num_employees)
	...
	SELECT			 [Teller] = SUM(Teller)
			...
			select		 [Datum] = @peildatum
						...
						,[Noemer]
						,[Laaddatum] = GETDATE()
						,Omschrijving =   'Verzuimfrequentie: '
										+ format(cast(Teller as float) / cast(Noemer as float), 'N2')
										+ '; '
										+ 'FTE: '
										+ format(Detail_01,'N1')
										+ '; '
...
-----------------------------------------------------------------------------------------------------------
	-- 1860 Maandelijks verzuimfrequentie

		set @fk_indicator_id = 1860;
...
-----------------------------------------------------------------------------------------------------------
	...
	IF exists(  select *
		...
		select	 Datum = @peildatum
				,Waarde = SUM(cast(Teller as float) / cast(Noemer as float))
							,Omschrijving =   'Verzuimfrequentie periode: '
...
-----------------------------------------------------------------------------------------------------------
	-- 1865 Jaarlijks verzuimfrequentie

		set @fk_indicator_id = 1865;
