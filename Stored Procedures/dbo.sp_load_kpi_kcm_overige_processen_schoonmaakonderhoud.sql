...
CREATE PROCEDURE[dbo].[sp_load_kpi_kcm_overige_processen_schoonmaakonderhoud](
	@peildatum date = '20191231'
)
as
/* #################################################################################################################

exec staedion_dm.[dbo].sp_load_kpi_kcm_overige_processen_schoonmaakonderhoud '20200430'
...
select @fk_indicator_id = min(id) from  [Dashboard].[Indicator] where lower([Omschrijving]) like '%schoonmaak%'
