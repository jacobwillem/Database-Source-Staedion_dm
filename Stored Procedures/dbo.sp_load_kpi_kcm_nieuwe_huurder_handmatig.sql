...
CREATE procedure [dbo].[sp_load_kpi_kcm_nieuwe_huurder_handmatig](
  @peildatum date = '20191231'
)
as
/* #################################################################################################################

exec staedion_dm.[dbo].[sp_load_kpi_kcm_nieuwe_huurder_handmatig] '20200131', @LoggenDetails=1
select * from empire_staedion_Data.etl.LogboekMeldingenProcedures
		declare @fk_indicator_id as smallint
		select @fk_indicator_id = min(id) from  [Dashboard].[Indicator] where lower([Omschrijving]) like '%tevred%nieuwe%huurders%'
  ...
  -- Standaard bewaren voor kpi's de details wel
	if @LoggenDetails = 1
		begin 		
			...
			SELECT Datum, [Score], getdate(), @fk_indicator_id, [sleutel eenheid], convert(nvarchar(7),[clusternr])
