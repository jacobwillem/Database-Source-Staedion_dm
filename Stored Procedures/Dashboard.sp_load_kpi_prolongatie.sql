...
SET ANSI_NULLS ON
GO
CREATE procedure [Dashboard].[sp_load_kpi_prolongatie] 
...
--------------------------------------------------------------------------------------------------------------------------------
TESTEN 
	exec [Dashboard].[sp_load_kpi_prolongatie] null, '2022-01-01'
--------------------------------------------------------------------------------------------------------------------------------
...
begin try
	...
	into #lst
	from [Dashboard].[Indicator] ind inner join (values (1560)) lst(indicator_id)
	...
	*/
	...
	; with prl ([Eenheidnr], [Klantnr], [Factuurtype], [Transactiewijze], [Bedrag], [Bedrag incl btw])
	as (select hdr.[Eenheidnr_], hdr.[Bill-to Customer No_], hdr.[Factuurtype], hdr.[Transaction Mode Code], sum(lne.[Amount]) [Bedrag], sum(lne.[Amount Including VAT]) [Bedrag incl btw]
		from [S-LOGSH-PROD].[Empire].[dbo].[Staedion$Prol Header] hdr inner join [S-LOGSH-PROD].[Empire].[dbo].[Staedion$Prol Line] lne
		on hdr.[No_] = lne.[Document No_]
		where hdr.[Posting Date] between dateadd(d, 1 - day(@peildatum), @peildatum) and eomonth(@peildatum)
		group by hdr.[Eenheidnr_], hdr.[Bill-to Customer No_], hdr.[Factuurtype], hdr.[Transaction Mode Code])
	select [Eenheidnr], [Klantnr], [Factuurtype], [Transactiewijze], [Bedrag], [Bedrag incl btw]
	into #prl 
	from prl

	insert into [staedion_dm].[Dashboard].[Realisatie] ([fk_indicator_id], [Datum], [Waarde], [Laaddatum])
		-- 1560 Geprolongeerd bedrag
		select 1560 [fk_indicator_id], eomonth(@peildatum) [Datum], sum(prl.[Bedrag]) / 1000.0, getdate() [Laaddatum]
		from #prl prl
		where (@IndicatorID = 1560 or @IndicatorID is null)

	; with prl ([Soort], [Transactiewijze], [Bedrag], [Aantal]) 
	as (select 'Verhuur' [Soort], prl.[Transactiewijze], sum(prl.[Bedrag]) [Bedrag], count(distinct [Eenheidnr]) 
		from #prl prl
		where prl.[Klantnr] <> ''
		group by prl.[Transactiewijze]
		union
		select 'Leegstand', '' [Transactiewijze], sum(prl.[Bedrag]) [Bedrag], count(distinct [Eenheidnr]) Aantal
		from #prl prl
		where prl.[Klantnr] = '')
	insert into [staedion_dm].[Dashboard].[RealisatieDetails] ([fk_indicator_id], [Datum], [Laaddatum], [Waarde], [Teller], [Noemer], [Omschrijving], 
		[Detail_01], [Detail_02], [Detail_03], [Detail_04])
		select 1560 [fk_indicator_id], eomonth(@peildatum) [Datum], getdate() [Laaddatum], prl.[Bedrag] / 1000.0 [Waarde], null [Teller], null [Noemer], prl.[Soort] + '; ' + prl.[Transactiewijze] [Omschrijving], 
			prl.[Soort] [Detail_01],
			prl.[Transactiewijze] [Detail_02], 
			format(prl.[Bedrag], '#,##0.00', 'NL-nl') [Detail_03],
			format(prl.[Aantal], '#,##0', 'NL-nl') [Detail_04]			
		from prl
		where (@IndicatorID = 1560 or @IndicatorID is null)
