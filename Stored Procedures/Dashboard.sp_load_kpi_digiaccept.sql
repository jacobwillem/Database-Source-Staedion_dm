...
SET ANSI_NULLS ON
GO
CREATE procedure [Dashboard].[sp_load_kpi_digiaccept] 
...
--------------------------------------------------------------------------------------------------------------------------------
TESTEN 
	exec [Dashboard].[sp_load_kpi_digiaccept] null, '2022-01-01'
--------------------------------------------------------------------------------------------------------------------------------
...
begin try
	...
	into #lst
	from [Dashboard].[Indicator] ind inner join (values (1590), (1591), (1592), (1593)) lst(indicator_id)
	...
	*/
	...
	select iif(cle.[Document No_] like 'MVFCG%', 1, 0) [Digi OLA], cle.[Entry No_] [Volgnummer], cle.[Description] [Omschrijving],
		cle.[Document No_] [Documentnr], cle.[Document Date] [Boekdatum], cle.[Customer No_] [Klantnr], cle.[Eenheidnr_] [Eenheidnr], -dcl.[Amount (LCY)] [Bedrag]
	into #dig
	...
	cle.[Description] like 'Digiaccept%'

	insert into [staedion_dm].[Dashboard].[Realisatie] ([fk_indicator_id], [Datum], [Waarde], [Laaddatum])
		-- 1590 Bedrag ontvangen digi ola
		select 1590 [fk_indicator_id], eomonth(@peildatum) [Datum], sum(dig.[Bedrag]),  getdate() [Laaddatum]
		from #dig dig
		where dig.[Digi OLA] = 1 and
		(@IndicatorID = 1590 or @IndicatorID is null)
		union
		-- 1591 Aantal ontvangen digi ola
		select 1591 [fk_indicator_id], eomonth(@peildatum) [Datum], count(*), getdate() [Laaddatum]
		from #dig dig
		where dig.[Digi OLA] = 1 and
		(@IndicatorID = 1591 or @IndicatorID is null)
		union
		-- 1592 Bedrag ontvangen stornoflow
		select 1592 [fk_indicator_id], eomonth(@peildatum) [Datum], sum(dig.[Bedrag]),  getdate() [Laaddatum]
		from #dig dig
		where dig.[Digi OLA] = 0 and
		(@IndicatorID = 1592 or @IndicatorID is null)
		union
		-- 1593 Aantal ontvangen stornoflow
		select 1593 [fk_indicator_id], eomonth(@peildatum) [Datum], count(*), getdate() [Laaddatum]
		from #dig dig
		where dig.[Digi OLA] = 0 and
		(@IndicatorID = 1593 or @IndicatorID is null)
	...
	insert into [staedion_dm].[Dashboard].[RealisatieDetails] ([fk_indicator_id], [Datum], [Laaddatum], [Waarde], [Teller], [Noemer], [Omschrijving], 
		...
		select 1590 [fk_indicator_id], eomonth(@peildatum) [Datum], getdate() [Laaddatum], dig.[Bedrag] / 1000.0 [Waarde], null [Teller], null [Noemer], dig.[Klantnr] + '; ' + dig.[Eenheidnr] [Omschrijving], 
		...
		(@IndicatorID = 1590 or @IndicatorID is null)
		union
		select 1591 [fk_indicator_id], eomonth(@peildatum) [Datum], getdate() [Laaddatum], 1 [Waarde], null [Teller], null [Noemer], dig.[Klantnr] + '; ' + dig.[Eenheidnr] [Omschrijving], 
			format(dig.[bedrag], '#,##0.00', 'NL-nl') [Detail_01],  
			format(dig.[Boekdatum], 'dd-MM-yyyy') [Detail_02], 
			dig.[Documentnr] [Detail_03], 
			dig.[Volgnummer] [Detail_04], 
		...
		(@IndicatorID = 1591 or @IndicatorID is null)
		union
		select 1592 [fk_indicator_id], eomonth(@peildatum) [Datum], getdate() [Laaddatum], dig.[Bedrag] / 1000.0 [Waarde], null [Teller], null [Noemer], dig.[Klantnr] + '; ' + dig.[Eenheidnr] [Omschrijving], 
			format(dig.[bedrag], '#,##0.00', 'NL-nl') [Detail_01],  
			format(dig.[Boekdatum], 'dd-MM-yyyy') [Detail_02], 
			dig.[Documentnr] [Detail_03], 
			dig.[Volgnummer] [Detail_04], 
		...
		(@IndicatorID = 1592 or @IndicatorID is null)
		union
		select 1593 [fk_indicator_id], eomonth(@peildatum) [Datum], getdate() [Laaddatum], 1 [Waarde], null [Teller], null [Noemer], dig.[Klantnr] + '; ' + dig.[Eenheidnr] [Omschrijving], 
			format(dig.[bedrag], '#,##0.00', 'NL-nl') [Detail_01],  
			format(dig.[Boekdatum], 'dd-MM-yyyy') [Detail_02], 
			dig.[Documentnr] [Detail_03], 
			dig.[Volgnummer] [Detail_04], 
