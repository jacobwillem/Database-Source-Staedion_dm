...
SET ANSI_NULLS ON
GO
create PROCEDURE[dbo].[sp_load_kpi_automatische_incasso_alternatief](
  @peildatum date
)
as
begin
	...
	begin try
		...
		select @fk_indicator_id = max(id) from  [Dashboard].[Indicator] where [Omschrijving] like '%Percentage%automatische%incasso%voor%woningen%'
		...
		insert into Dashboard.[RealisatieDetails](
			...
			-- Verzamelfacturen uitsluiten
			where cus.[Combine Shipments] = 0 and
			-- Indicator is alleen voor woningen
			-- in filter worden zelfstandige en onzelfstandige woningen opgehaald
			typ.[Analysis Group Code] in ('WON ZELF', 'WON ONZ') and
			-- klanten met prolongatiefactuur in de maand van de peildatum met afwachtcode uitsluiten
			adi.[Customer No_] not in (
				select cle.[Customer No_]
				from empire_data.dbo.Staedion$Cust__Ledger_Entry cle
				where cle.[Posting Date] between datefromparts(year(@peildatum), month(@peildatum), 1) and eomonth(@peildatum) and
				cle.[Source Code] = 'PROLON' and cle.[On Hold] in ('BR', 'CON', 'DON', 'DW', 'FIM', 'NA', 'NU', 'PSH', 'S&S', 'SSSF', 'VHZ', 'WSNP'))
