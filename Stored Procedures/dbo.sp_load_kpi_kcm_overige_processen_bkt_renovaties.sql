...
CREATE PROCEDURE[dbo].[sp_load_kpi_kcm_overige_processen_bkt_renovaties](
...
declare @fk_indicator_id as smallint
	...
	select * from  [Dashboard].[Indicator] where lower([Omschrijving]) like '%BKT%renovaties%'
...
select * from staedion_dm.Dashboard.[RealisatieDetails] where fk_indicator_id =  @fk_indicator_id 
select * from staedion_dm.Dashboard.vw_Indicator where id =  @fk_indicator_id or id = 1201
select max(Datum), count(*) from staedion_dm.Dashboard.[Realisatie] where fk_indicator_id =  @fk_indicator_id
delete from Dashboard.[Realisatie] where fk_indicator_id = 1201
delete from Dashboard.[RealisatieDetails] where fk_indicator_id = 1201
----------------------------------------------------------------------------------------------------------------
WIJZIGINGEN
----------------------------------------------------------------------------------------------------------------
...
begin try
	...
	select @fk_indicator_id = max(id) from  [Dashboard].[Indicator] where lower([Omschrijving]) like '%dienstverlening%BKT%renovaties%'
