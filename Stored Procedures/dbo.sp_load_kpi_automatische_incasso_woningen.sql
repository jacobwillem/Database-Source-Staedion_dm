...
SET ANSI_NULLS ON
GO
create PROCEDURE[dbo].[sp_load_kpi_automatische_incasso_woningen](
  @peildatum date
)
as
begin
	...
	begin try
		...
		select @fk_indicator_id = min(id) from  [Dashboard].[Indicator] where [Omschrijving] like '%Percentage%automatische%incasso%woningen%'
