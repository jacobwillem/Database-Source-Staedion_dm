...
CREATE procedure [Datakwaliteit].[sp_vve-status] (@fk_indicatordimensie_id as int = null)   
AS
/* ###################################################################################################
...
--------------------------------------------------------------------------------------------------------------
				if @fk_indicatordimensie_id = 18 -- is de VVE status 0 (Leeg), 1 (Actief) of 2 (Slapend)
					begin
							insert into Datakwaliteit.RealisatieDetails (Eenheidnr
								...
								select distinct  Eenheidnr = co_vve.Eenheidnr_
												,Omschrijving = 'VVE-cluster = ' + c_vve.Nr_ + ';  VVE status = ' + cast(c_vve.[Status VvE] as varchar)
												,Bevinding = 'VVE status is niet 0 (Leeg), 1 (Actief) of 2 (Slapend)'
												,Laaddatum = @Laaddatum
								...
								where c_vve.Clustersoort = 'VVE' and c_vve.[Status VvE] not in (0, 1, 2)
