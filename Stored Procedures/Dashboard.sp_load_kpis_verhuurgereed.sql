...
CREATE PROCEDURE [Dashboard].[sp_load_kpis_verhuurgereed] (
  @peildatum DATE = '20210131', @WelkeChildPackageUitvoeren AS NVARCHAR(MAX) =  
  N'[{"IndicatorID": 2041, "UitvoerenJaNee": "Ja"},
  {"IndicatorID": 2042, "UitvoerenJaNee": "Ja"},
  {"IndicatorID": 2061, "UitvoerenJaNee": "Ja"},
  {"IndicatorID": 2062, "UitvoerenJaNee": "Ja"},
  {"IndicatorID": 2063, "UitvoerenJaNee": "Ja"},
  {"IndicatorID": 2064, "UitvoerenJaNee": "Ja"},
  {"IndicatorID": 2065, "UitvoerenJaNee": "Ja"}]'
)
AS
/* #############################################################################################################################
...
----------------------------------------------------------------------------------- 
		...
		SELECT   GOM.*
				...
				,[Omschrijving] =	  'Onderhoudsorder: '
									...
									+ '; '
									+ 'Acceptatiedatum: '
									+ format(GOM.[Order - acceptatiedatum], 'yyyy-MM-dd')
									+ '; '
									+ 'Verzoek ingevoerd vanuit: '
									+ GOM.[Verzoek - Ingevoerd vanuit]
									+ '; '
									+ 'Verhuurteam: '
									+ GOM.[Verhuurteam]
									+ '; '
									...
									+ '; '
									+ 'Clusternummer: '
									+ GOM.[Verzoek - clusternummer]
									+ '; '
									+ 'Adres: '
									+ GOM.[Adres]
		INTO #GOM
		FROM [staedion_dm].[Verhuur].[GeaccepteerdeOrdersMutatieonderhoud] AS GOM
		LEFT OUTER JOIN [empire_data].[dbo].[Vendor] AS LEV
						ON LEV.[No_] = GOM.[Order - leverancier]
		;
...
----------------------------------------------------------------------------------- 
		...
		Declare kpi CURSOR FOR 
			...
			SELECT	CONVERT(DATETIME ,GETDATE(), 120) AS [Laaddatum]
					...
					,GOM.[Omschrijving] AS [Omschrijving] 
					,GOM.[DagelijksOnderhoudGebied] AS [Detail_01]
					,GOM.[Verhuurteam] AS [Detail_02]
					,GOM.[Functie] AS [Detail_03]
					,GOM.[Inspecteur] AS [Detail_04]
					,GOM.[Technischtype] AS [Detail_05]
					,GOM.[Order] AS [Detail_06] 
					,GOM.[Order - status] AS [Detail_07]
					,GOM.[Leveranciersnaam] AS [Detail_08]
					,NULL AS [Detail_09]
					,NULL AS [Detail_10]
					,GOM.[Verzoek - eenheidnr] AS [eenheidnummer]
					,NULL AS [bouwbloknummer]
					,GOM.[Verzoek - clusternummer] AS [clusternummer]
					,GOM.[Verzoek - huurdernr] AS [klantnummer]
					,NULL AS [volgnummer]
					...
					,GOM.[Order - leverancier] AS [leveranciernummer]
					,NULL AS [werknemernummer]
					,NULL AS [projectnummer]
					,NULL AS [verzoeknummer]
					,GOM.[Order] AS [ordernummer]
					,GOM.[Taak] AS [taaknummer]
					,NULL AS [overig]
					FROM #GOM AS GOM
					WHERE (
						(GOM.[Taak - standaard taakcode] = 'MU-01' AND @Cursor_indicatorid = 2041)
						OR
						(GOM.[Taak - standaard taakcode] = 'MU-02' AND @Cursor_indicatorid = 2042)
						OR
						(GOM.[Taak - standaard taakcode] = 'MU-03' AND @Cursor_indicatorid = 2061)
						OR
						(GOM.[Taak - standaard taakcode] = 'MU-04' AND @Cursor_indicatorid = 2062)
						OR
						(GOM.[Taak - standaard taakcode] = 'MU-05' AND @Cursor_indicatorid = 2063)
						OR
						(GOM.[Taak - standaard taakcode] = 'MU-06' AND @Cursor_indicatorid = 2064)
						OR
						(GOM.[Taak - standaard taakcode] = 'MU-07' AND @Cursor_indicatorid = 2065)
					)
					AND EOMONTH(GOM.[Order - acceptatiedatum]) = EOMONTH(@Peildatum)
