...
CREATE procedure [dbo].[sp_load_kpi_personeelslasten_budget](
  @peildatum date = '20210131'
)
as
/* #################################################################################################################

exec staedion_dm.[dbo].[sp_load_kpi_personeelslasten_budget] '20210131'
...
select max(Datum), count(*) from staedion_dm.Dashboard.[NormenDetails] where fk_indicator_id = @fk_indicator_id
...
delete from Dashboard.[NormenDetails] where fk_indicator_id = @fk_indicator_id 

1900	Personeelslasten gebaseerd op (1700 Bedrijfslasten volgens Aedes Benchmark (per VHE))
...
BEGIN TRY
  ...
  -- Diverse variabelen
		...
		select @budget = 'BEGR ' + format(@peildatum, 'yyyy')

  -- Standaard bewaren voor kpi's de details wel
	if @LoggenDetails = 1
		begin 		
			delete from Dashboard.[NormenDetails] where fk_indicator_id between @fk_indicator_id_min and @fk_indicator_id_max and Datum >= dateadd(d, 1-day(@peildatum), @peildatum)
			...
			[Grootboek] as 
				(
					...
					where		 [Rekeningnummer]	IN ( 'A411100'
														...
														,'A416400')
					and			 [Budget] = @budget
					and			 [Datum] >= dateadd(d, 1-day(@peildatum), @peildatum)
			...
			insert into [Dashboard].[NormenDetails]
						([Datum]
						,[Waarde]
						,[Laaddatum]
						,[Omschrijving]
		   ...
		   -- Samenvatting opvoeren tbv dashboards
			...
			insert into Dashboard.[Normen] (
				...
				select det.fk_indicator_id, det.Datum, sum(det.[Waarde]*1.00)
				from Dashboard.[NormenDetails] det
				where det.fk_indicator_id between @fk_indicator_id_min and @fk_indicator_id_max and det.Datum >= dateadd(d, 1-day(@peildatum), @peildatum)
				group by det.Datum, det.fk_indicator_id;
