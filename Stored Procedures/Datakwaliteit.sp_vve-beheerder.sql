...
BEGIN TRY
  ...
  -- Diverse variabelen
		...
		DECLARE @fk_indicator_id as int = 2185
		DECLARE @Entiteit as nvarchar(50) = 'Eenheid'
	    DECLARE @Attribuut as nvarchar(255) = 'VVE beheerder'
					...
					begin
							...
							insert into Datakwaliteit.RealisatieDetails (Eenheidnr
								...
								select distinct  Eenheidnr = co_vve.Eenheidnr_
												,Omschrijving = 'VVE-cluster = ' + c_vve.Nr_ + ';  VVE status = ' + cast(c_vve.[Status VvE] as varchar) + ';  VVE beheerder = ' + c_vve.Beheerder
												,Bevinding = 'VVE beheerder is niet leeg terwijl de VVE status 2 (Slapend) is'
												,Laaddatum = @Laaddatum
								...
								where c_vve.Clustersoort = 'VVE' and c_vve.[Status VvE] = 2 and c_vve.Beheerder <> ''

								UNION ALL

								select distinct  Eenheidnr = co_vve.Eenheidnr_
												,Omschrijving = 'VVE-cluster = ' + c_vve.Nr_ + ';  VVE status = ' + cast(c_vve.[Status VvE] as varchar) + ';  VVE beheerder = ' + iif(c_vve.Beheerder = '' or c_vve.Beheerder is null, 'ONBEKEND/NVT', c_vve.Beheerder)
												,Bevinding = 'VVE beheerder is leeg terwijl de VVE status 1 (Actief) is'
												,Laaddatum = @Laaddatum
												,fk_indicator_id = @fk_indicator_id
												,fk_indicatordimensie_id = @fk_indicatordimensie_id
								from empire_data.dbo.Staedion$Cluster c_vve
								left join empire_data.dbo.Staedion$Cluster_OGE co_vve on co_vve.Clusternr_ = c_vve.Nr_
								where c_vve.Clustersoort = 'VVE' and c_vve.[Status VvE] = 1 and c_vve.Beheerder = ''
