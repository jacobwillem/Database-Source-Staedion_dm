...
CREATE procedure [dbo].[sp_load_kpi_kcm_woonkwaliteit_lager_dan_handmatig](
  @peildatum date = '20191231'
)
as
/* #################################################################################################################

exec staedion_dm.[dbo].[sp_load_kpi_kcm_woonkwaliteit_lager_dan_handmatig] '20200131',@LoggenDetails =1
select distinct Databaseobject, ErrorMessage from empire_staedion_Data.etl.LogboekMeldingenProcedures where TijdMelding >= '20200501'  

declare @fk_indicator_id as smallint
select @fk_indicator_id = min(id) from  [Dashboard].[Indicator] where lower([Omschrijving]) like '%eoordeelt%kwaliteit%<6%'
select max(Datum), count(*) from staedion_dm.Dashboard.[RealisatieDetails] where fk_indicator_id = @fk_indicator_id and year(Datum) = 2020
select max(Datum), count(*) from staedion_dm.Dashboard.[Realisatie] where fk_indicator_id = @fk_indicator_id and year(Datum) = 2020
select * from Dashboard.[Realisatie] where fk_indicator_id = @fk_indicator_id and year(Datum) = 2020
select * from Dashboard.[RealisatieDetails] where fk_indicator_id = @fk_indicator_id and year(Datum) = 2020

Versie 2: JvdW 20200612 - Conversion failed when converting date and/or time from character string.
----------------------------------------------------------------------------------------------------------------
WIJZIGINGEN
----------------------------------------------------------------------------------------------------------------
  ...
  -- Standaard bewaren voor kpi's de details wel
	if @LoggenDetails = 1
		begin 		
			...
			SELECT Datum, [Score woningkwaliteit < 6],getdate(), convert(varchar(10), Datum, 120) + ' ; ' + convert(varchar(20), Tijdstip, 120) + ' ; ' + Postcode + ' ; ' + isnull(Eenheidnr, ''), @fk_indicator_id, [sleutel eenheid]
