...
CREATE procedure [dbo].[sp_load_kpi_energie_vernieuwde_labels_energetische_verbetering] (@peildatum date )
/**************************************************************************************************************
Door	MV
...
begin
	...
	begin try 
		...
		select @fk_indicator_id = min(id) from  [Dashboard].[Indicator] where lower([Omschrijving]) like '%energielabels ivm energetische verbetering'
	...
	if @LoggenDetails = 1
		begin 		
			...
			;
				...
				LEFT JOIN [Algemeen].[Energielabel verlopen] AS VAB
				ON VAB.eenheidnr = ELA.Eenheidnummer
				WHERE VAB.eenheidnr is null 
				--AND eomonth(ELA.[Datum]) = eomonth(@peildatum)
