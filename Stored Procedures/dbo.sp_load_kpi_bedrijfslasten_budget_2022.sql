...
CREATE procedure [dbo].[sp_load_kpi_bedrijfslasten_budget_2022](
  @peildatum date = '20220131'
)
as
/* #################################################################################################################

exec staedion_dm.[dbo].[sp_load_kpi_bedrijfslasten_budget_2021] '20210131'
...
BEGIN TRY
  ...
  -- Diverse variabelen
		...
		declare @peildatum_eenheden as date

		set	@start = current_timestamp
		select @peildatum_eenheden = convert(date, dateadd(yy, datediff(yy, 0, @peildatum), -1))
  ...
  -- Standaard bewaren voor kpi's de details wel
	if @LoggenDetails = 1
		begin 		
			delete from Dashboard.[Normen] where fk_indicator_id between @fk_indicator_id_min and @fk_indicator_id_max and datum >= dateadd(d, 1-day(@peildatum), @peildatum)
				...
				(		
					select		 [Rekeningnummer]				= 'A814945'
								...
								,[Bedrag]						= convert(float, (
																sum(iif([Rekeningnummer] like 'A4%' and [Rekeningnummer] <> 'A499999',	[Bedrag], 0)) * 0.2713 + 
																...
																sum(iif([Rekeningnummer] in ('A570200'),								[Bedrag], 0)) * 0.1000		-- A570200 = Dekking doorberekende uren LF
																))
					from		 [Grootboek]
					group by	 [Datum]
					...
					union select [Rekeningnummer]				= 'A830945'
								...
								,[Datum]						--= @peildatum
								,[Bedrag]						= convert(float, sum([Bedrag]) * 0.0043)
					from		 [Grootboek]
					...
					union select [Rekeningnummer]				= 'A812945'
								...
								,[Datum]						--= @peildatum
								,[Bedrag]						= convert(float, (
																	sum(iif([Rekeningnummer] like 'A4%' and [Rekeningnummer] <> 'A499999',	[Bedrag], 0)) * 0.0314 + 
																	sum(iif([Rekeningnummer] in ('A515220'),								[Bedrag], 0)) * 0.2765		-- A515220 = Dekking direct loon
																	))
					from		 [Grootboek]
					group by	 [Datum]
					...
					union select [Rekeningnummer]				= 'A860945'
								...
								,[Datum]						--= @peildatum
								,[Bedrag]						= convert(float, (
																	sum(iif([Rekeningnummer] like 'A4%' and [Rekeningnummer] <> 'A499999',	[Bedrag], 0)) * 0.0705
																	))
					from		 [Grootboek]
					group by	 [Datum]
					...
					union select [Rekeningnummer]				= 'A870945'
								...
								,[Datum]						--= @peildatum
								,[Bedrag]						= convert(float, (
																	sum(iif([Rekeningnummer] like 'A4%' and [Rekeningnummer] <> 'A499999',	[Bedrag], 0)) * 0.1729 + 
																	sum(iif([Rekeningnummer] in ('A515100'),								[Bedrag], 0)) * 0.2500 + 	-- A515100 = Dekking doorberekende uren verhuur (herstructurering)
																	sum(iif([Rekeningnummer] in ('A570200'),								[Bedrag], 0)) * 0.6500		-- A570200 = Dekking doorberekende uren LF
																	))
					from		 [Grootboek]
					group by	 [Datum]
					...
					union select [Rekeningnummer]				= 'A815945'
								...
								,[Datum]						--= @peildatum
								,[Bedrag]						= convert(float, (
																	sum(iif([Rekeningnummer] like 'A4%' and [Rekeningnummer] <> 'A499999',	[Bedrag], 0)) * 0.4218 + 
																	sum(iif([Rekeningnummer] in ('A515100'),								[Bedrag], 0)) * 0.6500 +	-- A515100 = Dekking doorberekende uren verhuur (herstructurering) 
																	sum(iif([Rekeningnummer] in ('A515220'),								[Bedrag], 0)) * 0.7235 +	-- A515220 = Dekking direct loon
																	sum(iif([Rekeningnummer] in ('A560200'),								[Bedrag], 0)) +				-- A560200 = Dekking administratiekosten brandschade
																	sum(iif([Rekeningnummer] in ('A515240'),								[Bedrag], 0)) +				-- A515240 = Dekking doorberekende uren projecten OH
																	sum(iif([Rekeningnummer] in ('A570200'),								[Bedrag], 0)) * 0.2500		-- A570200 = Dekking doorberekende uren LF
																	))
					from		 [Grootboek]
					group by	 [Datum]
					...
					union select [Rekeningnummer]				= 'A850945'
								...
								,[Datum]						--= @peildatum
								,[Bedrag]						= convert(float, (
																	sum(iif([Rekeningnummer] like 'A4%' and [Rekeningnummer] <> 'A499999',	[Bedrag], 0)) * 0.0278 + 
			...
			[Details] as
				(
					...
					inner join		[empire_staedion_data].[dbo].[Bedrijfslasten schema rekening budget] r on r.fk_categorie_id = c.id
					inner join		(
									select			 [Rekeningnummer]
													,[Rekeningnaam]
													,[Rekening]	
			...
			[Weging] as
				(
					...
					*/
					...
					where		(o.[Einde exploitatie]  = '1753-01-01' or  o.[Einde exploitatie]  > @peildatum_eenheden)
					and			(o.[Begin exploitatie] <> '1753-01-01' and o.[Begin exploitatie] <= @peildatum_eenheden)
