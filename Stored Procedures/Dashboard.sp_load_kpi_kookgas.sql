...
CREATE PROCEDURE [Dashboard].[sp_load_kpi_kookgas] 
...
----------------------------------------------------------------------------------- 
		INSERT INTO [Dashboard].[RealisatieDetails]
			...
			SELECT	CONVERT(DATETIME ,GETDATE(), 120) AS [Laaddatum]
					,[aantal_g_huidig] AS [Waarde]
					,NULL AS [Teller]
					,NULL AS [Noemer]
					,IIF(YEAR(@Peildatum) = YEAR(DATEADD(YEAR, 1, [Peildatum])), EOMONTH(@peildatum), EOMONTH(DATEADD(YEAR, 1, [Peildatum]))) AS [Datum]
					,@IndicatorID AS [fk_indicator_id]
					,'Cluster: ' + [clusternr] + '; Totaal aantal adressen in cluster: ' + CAST([Aantal adressen] as nvarchar) +
									 '; Aantal in cluster met kookgas: ' + CAST([aantal_g_huidig] as nvarchar) +
									 '; Percentage in cluster met kookgas: ' + CAST(ROUND((([aantal_g_huidig]/[Aantal adressen]) * 100),0) as nvarchar) + '%' AS [Omschrijving] 
					,[clusternr] AS [Detail_01]
					,NULL AS [Detail_02]
					,NULL AS [Detail_03]
					,NULL AS [Detail_04]
					,NULL AS [Detail_05]
					...
					,NULL AS [taaknummer]
					,NULL AS [overig]
					FROM [EXCEL_STEDIN_KOOKGAS]...[Kookgas$]
					WHERE   [gem SJV Gas HUIDIG JAAR] <= 100
						AND [max SJV Gas HUIDIG JAAR] <= 100
						AND [gem SJV Gas HUIDIG JAAR] <> 65
						AND [max SJV Gas HUIDIG JAAR] <> 65
