...
CREATE PROCEDURE[dbo].[sp_load_kpi_kcm_overige_processen_servicekosten](
	@peildatum date = '20191231'
)
as
/* #################################################################################################################

exec staedion_dm.[dbo].[sp_load_kpi_kcm_overige_processen_servicekosten] '20200630'
select * from empire_staedion_Data.etl.LogboekMeldingenProcedures
select min(id) from  [Dashboard].[Indicator] where lower([Omschrijving]) like '%servicekosten%'
select max(Datum), count(*) from staedion_dm.Dashboard.[RealisatieDetails] where fk_indicator_id = 1207
select max(Datum), count(*) from staedion_dm.Dashboard.[Realisatie] where fk_indicator_id = 1207
----------------------------------------------------------------------------------------------------------------
WIJZIGINGEN
----------------------------------------------------------------------------------------------------------------
...
begin try
	...
	select @fk_indicator_id = min(id) from  [Dashboard].[Indicator] where lower([Omschrijving]) like '%servicekosten%'
	...
	insert into [Dashboard].[RealisatieDetails] (
		...
		select kcm.[INGEVULDE GEGEVENS] Datum, 
			convert(int, kcm.[U krijgt informatie bij de jaarafrekening van uw servicekosten#]) Waarde, 
			getdate(), 
			kcm.[Bouwbloknr] + ' ; ' + kcm.[Bouwbloknaam] + ' ; ' 
			--+ cast(cast(kcm.[InteractieID] as int) as varchar) + ' ; ' 
			--+ kcm.[Naam behandelend medewerker Staedion] 
			as [Omschrijving],
			@fk_indicator_id, convert(nvarchar(7),[clusternr])
		from empire_staedion_data.kcm.STN415_Ingevulde_gegevens kcm 
		where isnumeric(kcm.[U krijgt informatie bij de jaarafrekening van uw servicekosten#]) = 1 and
