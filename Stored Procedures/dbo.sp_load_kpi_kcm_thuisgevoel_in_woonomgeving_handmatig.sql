...
CREATE procedure [dbo].[sp_load_kpi_kcm_thuisgevoel_in_woonomgeving_handmatig](
  @peildatum date = '20210131'
)
as
/* #################################################################################################################

exec staedion_dm.[dbo].[sp_load_kpi_kcm_thuisgevoel_in_woonomgeving_handmatig] '20210131'
select * from empire_staedion_Data.etl.LogboekMeldingenProcedures
select max(Datum), count(*) from staedion_dm.Dashboard.[RealisatieDetails] where fk_indicator_id = 710 and year(Datum) = 2021
select max(Datum), count(*) from staedion_dm.Dashboard.[Realisatie] where fk_indicator_id = 710 and year(Datum) = 2021
select * from Dashboard.[Realisatie] where fk_indicator_id = 710 and year(Datum) = 2021
select * from Dashboard.[RealisatieDetails] where fk_indicator_id = 710 and year(Datum) = 2021
----------------------------------------------------------------------------------------------------------------
WIJZIGINGEN
----------------------------------------------------------------------------------------------------------------
20210201 JvdW: jaargang 2020 ongemoeid laten - vandaar extra conditie toegevoegd bij delete en insert
20210201 JvdW: [Indicator Voelt zich thuis in buurt] aangemaakt in view en hier aangeroepen
...
BEGIN TRY
  ...
  -- Diverse variabelen
		...
		select @fk_indicator_id = min(id) from  [Dashboard].[Indicator] where lower([Omschrijving]) like '%bewoners%voelen%thuis%woonomgeving'
  ...
  -- Standaard bewaren voor kpi's de details wel
	if @LoggenDetails = 1
		begin 		
			...
			SELECT Datum, [Indicator Voelt zich thuis in buurt], getdate(), convert(nvarchar(10),[Score thuisgevoel]), @fk_indicator_id, [sleutel eenheid], convert(nvarchar(7),[clusternr])
			-- indicator en score veld wijzigen!
			-- select top 10  * 
